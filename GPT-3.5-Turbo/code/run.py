...
def main():
    ...
    if args.do_train:
        ...
        finetuned_examples = EqDataset(args.codebase_data_file, args.train_data_file)
        dataset = EqDataset(args.codebase_data_file, args.test_data_file)

        # create fine-tunable dataset
        T1 = time.perf_counter()
        data = []
        for idx in range(len(finetuned_examples)):
            code_1 = finetuned_examples[idx][0]
            code_2 = finetuned_examples[idx][1]
            ...
            content = "Please identify if the two following codes are semantically equal. Please only answer 'yes' or 'no', 'yes' means they are semantically equal. 'no' means they are not. \n" + \
                      ...
                      code_2 + \
                      "\n'''"
            if finetuned_examples[idx][2] == 1:
                label = 'yes'
            else:
                label = 'no'
            msg = {"messages": [{"role": "system", "content": instruction},
                                {"role": "user", "content": content},
                                {"role": "assistant", "content": label}]}
            data.append(msg)
        finetunable_file = "{}/fine-tune_set.jsonl".format(args.output_dir)
        file = jsonlines.open(finetunable_file, "w")
        for i in range(len(data)):
            jsonlines.Writer.write(file, data[i])
        file.close()
        T2 = time.perf_counter()
        finetunable_set_gen_time = T2 - T1

        # upload fine-tunable set
        file_info = client.files.create(file=open(finetunable_file, "rb"), purpose="fine-tune")
        file_id = file_info.dict()['id']

        # start fine-tuning
        print('Fine-tuning had already started, it may take couple of hours. After finishing, the program will automatically make inference.\n')
        print('Note if you stop the program, the fine-tuning is still running on OpenAI server.\n')
        T1 = time.perf_counter()
        response = client.fine_tuning.jobs.create(training_file=file_id, model=model,
                                                  hyperparameters={"n_epochs": args.epoch, "learning_rate_multiplier": 1, "batch_size": args.train_batch_size})
        # waiting for fine-tuning job finish
        ft_job_id = response.dict()['id']
        while True:
            response = client.fine_tuning.jobs.retrieve(ft_job_id)
            status = response.dict()['status']
            if status == 'succeeded':
                ft_model = response.dict()['fine_tuned_model']
                print("Fine-tuning successful! Will start inference.")
                break
            elif status == 'failed':
                print("Fail to fine-tuning. Here is the reasons: {}".format(response['error']))
            else:
                print("Still fine-tuning. Will fetch ft-job status after 300 seconds.")
            time.sleep(300)    # Wait for a certain amount of time before checking again
        T2 = time.perf_counter()
        training_time = T2 - T1
        ...
        for idx in tqdm(range(len(dataset))):
            ...
            content = "Please identify if the two following codes are semantically equal. Please only answer 'yes' or 'no', 'yes' means they are semantically equal. 'no' means they are not. \n" + \
                      ...
                      code_2 + \
                      "\n'''"
            message = ChatCompletion(content, instruction, client, ft_model)
        ...
        print('Training Time: %s s' % (finetunable_set_gen_time + training_time))
        ...
        results_df.to_csv('{}/fine-tuned_gpt-3.5-turbo.csv'.format(args.output_dir), index=False)


    # zero-shot prompting or few-shot prompting
    if args.do_test:
        api_key = args.api_key
        client = OpenAI(api_key=api_key)
        dataset = EqDataset(args.codebase_data_file, args.test_data_file)
        ...
        if args.test_type == "zero-shot-prompt":
            ...
            results_dict = {'Messages': messages_list, 'Numerical_Pred': prediction_list}
            results_df = pd.DataFrame.from_dict(results_dict)
            results_df.to_csv('{}/zero-shot-prompt_gpt-3.5-turbo.csv'.format(args.output_dir), index=False)
            ...
            results_dict = {'Messages': messages_list, 'Numerical_Pred': prediction_list}
            results_df = pd.DataFrame.from_dict(results_dict)
            results_df.to_csv('{}/few-shot-prompt_gpt-3.5-turbo.csv'.format(args.output_dir), index=False)
        else:
